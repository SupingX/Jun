...
public class MainActivity extends Activity {
	...
	public void buttonOnClicked(View view) {
		...
		case R.id.btnCreateNotify:
			...
			mHandler.sendMessageDelayed(
					mHandler.obtainMessage(EVENT_SHOW_CREATE_NOS), 50);
			break;
		case R.id.btnClearLastNotify:
			logNLS("Clear Last notification...");
			clearLastNotification();
			mHandler.sendMessageDelayed(
					mHandler.obtainMessage(EVENT_LIST_CURRENT_NOS), 50);
			break;
	...
	private String getCurrentNotificationString() {
		String listNos = "";
		StatusBarNotification[] currentNos = NotificationMonitor
				.getCurrentNotifications();
		if (currentNos != null) {
			for (int i = 0; i < currentNos.length; i++) {
				Notification notification = currentNos[i].getNotification();
				Bundle b = notification.extras;
				CharSequence charSequence = b
						.getCharSequence(Notification.EXTRA_TEXT);
				CharSequence notificationTitle = b
						.getString(Notification.EXTRA_TITLE);
				Bitmap notificationLargeIcon = ((Bitmap) b
						.getParcelable(Notification.EXTRA_LARGE_ICON));
				Bitmap notificationSmallIcon = ((Bitmap) b
						.getParcelable(Notification.EXTRA_SMALL_ICON));
				CharSequence notificationText = b
						.getCharSequence(Notification.EXTRA_TEXT);
				CharSequence notificationSubText = b
						.getCharSequence(Notification.EXTRA_SUB_TEXT);
				
				
				listNos = "\n"+i + "___" + currentNos[i].getPackageName() + "_"
						+ currentNos[i].describeContents() + "_"
						+ currentNos[i].getPostTime() + "_"
						+ currentNos[i].getTag() + "_"
						+ currentNos[i].getUserId() + "_"
						+ charSequence + "_"
						+ notificationTitle + "_"
						+ notificationText + "_"
						+ notificationSubText + 
		...
		if (isEnabledNLS) {
			...
			if (n == 0) {
				result = getResources().getString(
						R.string.active_notification_count_zero);
			} else {
				result = String.format(getResources().getQuantityString(
						R.plurals.active_notification_count_nonzero, n, n));
			}
	...
	private void showCreateNotification() {
		if (NotificationMonitor.mPostedNotification != null) {
			String result = NotificationMonitor.mPostedNotification
					.getPackageName()
					+ "\n"
